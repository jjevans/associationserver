...
class Phenotype():
	...
	def desc_by_pheno(self,id):
		return self.hpo_gene_obj.desc_by_pheno(id)
	...
	def genes_by_pheno(self,id):
		return self.hpo_gene_obj.genes_by_pheno(id)
	
	def pheno_by_gene(self,id):
		
		response = self.hpo_gene_obj.pheno_by_gene(id)

		pheno = list()
		for res in response:
			desc = self.desc_by_pheno(res[0])
			pheno.append({"id":res[0],"desc":desc,"info":list()})
			
		return pheno

	def geneset(self):
		return self.hpo_gene_obj.geneset()
